  ...
  return (
    <group>
      ...
      {data.buildings.map((b) => (
        <RigidBody key={b.id} type="fixed" position={[b.x, b.h / 2 + SIDEWALK_HEIGHT, b.z]}>
      ...
      {/* 4. House Numbers (Text) */}
      {data.buildings.map((b) => (
        <Text
          key={`num-${b.id}`}
          position={[b.x, DOOR_HEIGHT + SIDEWALK_HEIGHT + 0.6, b.z + (b.d / 2 * b.face) + (0.3 * b.face)]}
          rotation={[0, b.face === 1 ? 0 : Math.PI, 0]}
          fontSize={1.4}
          color="#ffffff"
          anchorX="center"
          anchorY="middle"
