...
export function Car({ ref, keys, cameraControls }) {
  ...
  const carRef = ref;
  const { scene } = useGLTF('/models/trike.glb'); // your GLB
  ...
  const [enableNormalView, setEnableNormalView] = useState(false);
...
ctx.putImageData(normalImageData, width, height); // Position: (width, height) - bottom right
  ...
  useFrame((state, delta) => {
    ...
    if (keys.current["v"]){
      setEnableNormalView(!enableNormalView);
  ...
  return (
    <>
      ...
      {showModel && (
        ...
        <mesh visible={true}>
          <boxGeometry args={[1.25, 1, 2]} /> {/* Mclaren [4, 1, 9]*/}
       ...
       {isAI && (
  <> {/* <--- The "Parent" Fragment starts here */}
