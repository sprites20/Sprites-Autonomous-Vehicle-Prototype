...
const EXPLOSION_SOUND_PATH = "/models/audios/explosion.mp3"; 
...
function ParticleScene({ explosionsRef }) {
  useFrame((_, delta) => {
...
  const chunkSize = 16;
  ...
  const generateParticles = (count, origin, type) => {
    ...
    if (type === 'smoke') {
      forceMultiplier = 0.5; // Smoke is slower
      particleSize = params.radius * 2.0; // Smoke is bigger
      ...
      forceMultiplier = 0.8; // Debris is mid-speed
      particleSize = params.radius * 1.5; // Debris is mid-size
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (type === 'shrapnel') {
        // **UPDATED SHRAPNEL LOGIC**: EVEN TIGHTER X/Z spread for focused cone effect
        dir = new THREE.Vector3(
            (Math.random() * 2 - 1) * 0.05, // Very tight X spread (was 0.2)
            Math.random() * -1, // ALWAYS fire downwards (negative Y)
            (Math.random() * 2 - 1) * 0.05  // Very tight Z spread (was 0.2)
        ).normalize();
      } else {
  ...
  const origin = params.randomSpawn
    ? new THREE.Vector3(
        (Math.random() - 0.5) * 80,
        Math.random() * 3,
        (Math.random() - 0.5) * 80
...
useEffect(() => {
    ...
    const handleControlsChange = () => {
        if (selectedObjectData) {
            ...
            objects.current = objects.current.map(o =>
...
        {localPlayerRef.current && (
      ...
      <ParticleScene explosionsRef={explosionsRef} />
      {/*
      <CharacterController
        //key={player.id}
        ...
        {projectiles.map((p) => {
          ...
          return (
            ...
            >
              ...
              <FlameEmitter position={[0, -.5, 0]} count={10} />
